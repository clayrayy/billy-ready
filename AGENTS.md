...
- Keep Brevo's `email_address_check` honeypot at `display: none !important`. Merely positioning it offscreen can let contact-card autofill populate it, causing Brevo to silently discard a legitimate signup while returning its generic success response.
